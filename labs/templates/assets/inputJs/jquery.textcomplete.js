...
+function ($) {
  ...
  $.extend(Dropdown.prototype, {
    ...
    isSpace: function (e) {
      var modifiers = e.ctrlKey || e.altKey || e.metaKey || e.shiftKey;
      return !modifiers && (e.keyCode === 32)  // SPACE
	},
    isPageup: function (e) {
  ...
  //  var memo = {};
	...
	var somlang= "hindi"
     if(somlang=="hindi"){
	  ...
	  else if(somlang=="bengali"){
      if (memob[term]) {
        callback(memob[term]);
      } else {
        func.call(this, term, function (data) {
          memob[term] = (memob[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	  else if(somlang=="assamese"){
      if (memoa[term]) {
        callback(memoa[term]);
      } else {
        func.call(this, term, function (data) {
          memoa[term] = (memoa[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	   else if(somlang=="punjabi"){
      if (memop[term]) {
        callback(memop[term]);
      } else {
        func.call(this, term, function (data) {
          memop[term] = (memop[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	  else if(somlang=="tamil"){
      if (memota[term]) {
        callback(memota[term]);
      } else {
        func.call(this, term, function (data) {
          memota[term] = (memota[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	  else if(somlang=="telugu"){
      if (memote[term]) {
        callback(memote[term]);
      } else {
        func.call(this, term, function (data) {
          memote[term] = (memote[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	  else if(somlang=="kannada"){
      if (memok[term]) {
        callback(memok[term]);
      } else {
        func.call(this, term, function (data) {
          memok[term] = (memok[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	  else if(somlang=="malayalam"){
      if (memom[term]) {
        callback(memom[term]);
      } else {
        func.call(this, term, function (data) {
          memom[term] = (memom[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	  else if(somlang=="gujarati"){
      if (memog[term]) {
        callback(memog[term]);
      } else {
        func.call(this, term, function (data) {
          memog[term] = (memog[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	  else if(somlang=="oriya"){
      if (memoo[term]) {
        callback(memoo[term]);
      } else {
        func.call(this, term, function (data) {
          memoo[term] = (memoo[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	  else if(somlang=="marathi"){
      if (memoma[term]) {
        callback(memoma[term]);
      } else {
        func.call(this, term, function (data) {
          memoma[term] = (memoma[term] || []).concat(data);
          callback.apply(null, arguments);
        });
      }
	  }
	/*  else if (memo[term] && somlang=="bengali") {
        callback(memo[term]);
      } else {
        func.call(this, term, function (data) {
          memo[term] = (memo[term] || []).concat(data);
...
/*******************************************textcomplete connection***************************************/

$.fn.reverieEditor = function(options) { 
...
var nuOfsugg=settings.noOfSuggs;
var somlang= lang
return this.each(function() {
$(this).textcomplete([
                                {                              
							...
							var stringRequestObj = JSON.stringify(requestJson);
									  ...
									  $.ajax({
											 type : "POST",
											 url : 'http://laascloud.cloudapp.net/parabola/transliterateSuggJSON',
								    ...
								    if(statusCode == 1){
									var reqIdList = response.requestIdList;
									var reqId = reqIdList[0];
									var reqObj = response[""+reqId];
									...
									for (var i = 0; i < 5; i++) {
										...
										//	}
											    ...
											    403: function() {
												       console.log("Forbidden. Please provide a proper API Key");
											    }
										});     
                                  },
...
function buildUrl()        //select language textcomplete
{    
     
    var somlang= lang
	return somlang;	
}
function tokencache(revApi,term)
{
	 var somlang= "hindi"
